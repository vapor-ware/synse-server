...
This currently needs to run on the VEC leader to access auto-fan.
"""
...
from synse.vapor_common import http  # nopep8 # pylint: disable=wrong-import-position,import-error
...
def _get_synse_prefix():
    ...
    url = PROTOCOL + VEC_IP + ':' + str(SYNSE_PORT) + '/' + SYNSE_NAME + '/' \
        + version + '/'
...
def _get_devices_by_type(scan_results, device_type):
    ...
    :param scan_results: The JSON results from a synse scan.
    :param device_type: The Synse device_type we are looking for.
    :returns: A list of devices in the scan results that match device_type. Each
    item in the list is a dict with keys device_type, rack_id, board_id,
    device_id which can be used to form the Synse URL to access the device.
    """
    result = list()
...
def _read_sensors(sensors):
    ...
    :param sensors: The sensors to read.
    :returns: An array of readings from Synse for the given sensors.
    """
    results = []
...
def _find_max_temperature_reading(readings):
    ...
    :param readings: A list of temperature readings from Synse.
    :returns: (int) The maximum temperature reading in degrees C.
    """
    result = -sys.maxint - 1
...
# List of colors to set the LEDs to. Key is the friendly name, value is the RGB
# color.
COLORS = OrderedDict(
    [
        ('off', 0x000000),
...
def _color_to_string(color):
    ...
    :param color: RGB value in COLORS.
    :returns: The friendly name (key) in COLORs or the RGB value if not found.
    """
...
def _set_led_color(led_controllers, color):
    ...
    :param led_controllers: The LED controllers on the VEC. Currently there is
    only one.
    :param color: The friendly name (key) in COLORS to set the LED to.
    :raises: Synse cannot be connected to.
    """
...
def _test_led_colors(led_controller):
    """Simple test for setting all colors in COLORS on the LED controller.
    :param led_controller: The LED controller to set the colors on.
    """
    logger.debug('')
...
def _set_fan_speed(rpm):
    ...
    :param rpm: The fan speed in RPM.
    :returns: rpm
    :raises: Synse cannot be connected to.
    """
...
FAN_SPEED_INITIAL = 175  # Fan speed setting before the first test loop.
FAN_SPEED_STEP = 25      # Fan step in rpm on each test loop.
...
def main():
    ...
    fan_controllers = _get_devices_by_type(scan_results, 'vapor_fan')  # FUTURE: Use.
    ...
    _set_fan_speed(0)   # NOTE: Should really put it back in auto,
