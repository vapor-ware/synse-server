#!/usr/bin/env python
""" OpenDCRE Southbound IPMI Emulator Throughput Tests
...
class IPMIEmulatorFailureThroughputTestCase(unittest.TestCase):
    ...
    def test_004_test_read(self):
        ...
        url = PREFIX + '/read/temperature/rack_1/40000000/0666'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_005_test_power(self):
        ...
        url = PREFIX + '/power/rack_1/40000000/0666/status'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_006_test_power(self):
        ...
        url = PREFIX + '/power/rack_1/40000000/0666/off'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_007_test_power(self):
        ...
        url = PREFIX + '/power/rack_1/40000000/0666/on'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_008_test_asset_info(self):
        ...
        url = PREFIX + '/asset/rack_1/40000000/0666'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_009_test_boot_target(self):
        ...
        url = PREFIX + '/boot_target/rack_1/40000000/0666'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_010_test_boot_target(self):
        ...
        url = PREFIX + '/boot_target/rack_1/40000000/0666/pxe'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_011_test_boot_target(self):
        ...
        url = PREFIX + '/boot_target/rack_1/40000000/0666/hdd'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_012_test_location(self):
        ...
        url = PREFIX + '/location/rack_1/40654321'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_013_test_led(self):
        ...
        url = PREFIX + '/led/rack_1/40000000/0666'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_014_test_led(self):
        ...
        url = PREFIX + '/led/rack_1/40000000/0666/on'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_015_test_led(self):
        ...
        url = PREFIX + '/led/rack_1/40000000/0666/off'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_016_test_fan(self):
        ...
        url = PREFIX + '/fan/rack_1/40000000/0666'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
    ...
    def test_017_test_host_info(self):
        ...
        url = PREFIX + '/host_info/rack_1/40000000/0666'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)

        self.assertEqual(len(responses), self.COUNT)
        for resp in responses:
            # while the this is pretty much just info from the BMC config, so should not fail
            self.assertEqual(resp, 200)

    def test_018_test_scan(self):
    ...
    def test_019_test_scan(self):
        ...
        url = PREFIX + '/scan/rack_1/40654321'
        async.map([async.get(url, hooks={'response': [_test_hook]}, timeout=self.TIMEOUT) for _ in xrange(self.COUNT)], size=self.SIZE)
