...
test suites within the opendcre_southbound directory. (the test suite files have the
...
The ./test directory also contains platform directories (macos, rpi, arm64) which contain the
...
The tests are run from the southbound-api directory from the Makefile.

  $ make macos-test

Additionally, the Makefile contains an option to remove the test containers.

  $ make macos-test-clean
