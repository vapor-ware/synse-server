#!/usr/bin/env python
""" OpenDCRE Southbound Redfish Device
...
class RedfishDevice(LANDevice):
    ...
    def _read(self, command):
        ...
        if device_type_string.lower() == 'fan_speed':
            ...
            raise OpenDCREException('{} not a supported device type for Redfish.'.format(device_type_string))
        ...
        try:
            ...
            raise OpenDCREException('No sensor reading returned from Redfish server.')
        except ValueError as e:
            logger.error('Error reading Redfish sensor: {}'.format(e.message))
            logger.exception(e)
            raise OpenDCREException('Error reading Redfish sensor: {}'.format(e.message))
    ...
    def _power(self, command):
        ...
        try:
            ...
            raise OpenDCREException('No response from Redfish server for power control action.')
    ...
    def _asset(self, command):
        ...
        try:
            ...
            raise OpenDCREException('No response from Redfish server when retrieving asset information via Redfish.')
        except ValueError as e:
    ...
    def _boot_target(self, command):
        ...
        try:
            ...
            raise OpenDCREException('No response from Redfish server on boot target operation via Redfish.')
        ...
        try:
            ...
            if led_state is not None and led_state.lower() != 'no_override':
                if led_state.lower() in ['on', 'off']:
                    ...
                    raise OpenDCREException('Unsupported state change to Redfish server on LED operation.')
            else:
            ...
            raise OpenDCREException('No response from Redfish server on LED operation.')
    ...
    def _fan(self, command):
        ...
        try:
            ...
            raise OpenDCREException('No response from Redfish server on fan operation via Redfish.')
