#!/usr/bin/env python
""" Redfish Connection Methods for OpenDCRE Redfish Bridge
...
def get_data(link, timeout, username=None, password=None):
    ...
    try:
        ...
        raise OpenDCREException('Unable to GET link {} due to ConnectionError: {}'.format(link, e.message))
...
def _get_members(json_data, ip_address, port):
    ...
    if 'Members' in json_data:
        try:
            ...
            raise OpenDCREException('Unable to verify number of members. {} not present.'.format(e.message))
    else:
...
def find_links(ip_address, port, timeout, username, password):
    ...
    try:
        ...
        raise OpenDCREException('Cannot retrieve {} collection links from root schema: {}.'.format(unfound, e.message))
    ...
    try:
        ...
        raise OpenDCREException('Cannot retrieve links from collections: {}. Bad path to {}.'.format(unfound, e.message))
