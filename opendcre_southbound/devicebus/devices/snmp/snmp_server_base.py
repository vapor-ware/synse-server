#!/usr/bin/env python
""" OpenDCRE Southbound Base class for specific SNMP server implementations.
...
class SnmpServerBase(object):
    ...
    def __init__(self, app_cfg, kwargs):
        ...
        # Derived classes should add to the command map for whatever OpenDCRE
    ...
    def _scan_internal(self):
        ...
        field that allows OpenDCRE to map an API call to an OpenDCRE device
        ...
        OpenDCRE to support multiple SNMP tables on one SNMP server.
    ...
    # region Common OpenDCRE commands.

    def _scan(self, command):
        """OpenDCRE API scan command implementation for an SNMP server.
        ...
        if not client_rack:
            ...
            # the board id does not exist, the OpenDcre server is not serving
            # anything at the URL specified by the client.
            # Therefore 404 Not Found.
            raise OpenDCREException("No rack with id {}.".format(client_rack_id))
        ...
        if client_board_id is not None:
            ...
            if not boards:
                raise OpenDCREException('No board with id {}.'.format(client_board_id))
            else:
                rack['boards'] = boards
        return Response(command=command, response_data=public_scan_results)

    def _scan_all(self, command):
        """OpenDCRE API scan all command implementation for an SNMP server.
        ...
        if force:
            try:
                ...
                # Device scan failures cannot take down OpenDCRE.
    ...
    def _version(self, command):
        ...
        if not board:
            ...
            # anything at the URL specified by the client.
            # Therefore 404 Not Found.
            raise OpenDCREException("No board on rack {} with id {}.".format(
    ...
    def get_device_from_command(self, command):
        ...
        if not board:
            ...
            # anything at the URL specified by the client.
            # Therefore 404 Not Found.
            raise OpenDCREException("No board with id {}.".format(board_id))

        device_id = '{0:04x}'.format(command.data['device_id'])
        device = SnmpServerBase.get_device(board, device_id)
        if not device:
            # Future: This is a REST 404. Since the board id is in the URL and
            # the device id does not exist, the OpenDcre server is not serving
            # anything at the URL specified by the client.
            # Therefore 404 Not Found.
            raise OpenDCREException("No device with id {}.".format(device_id))
    ...
    def run_command(self, command):
        """ Run an incoming OpenDCRE command.
