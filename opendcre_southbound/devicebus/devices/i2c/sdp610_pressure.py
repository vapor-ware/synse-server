#!/usr/bin/env python
""" OpenDCRE Southbound I2C pressure via SDP610
...
class SDP610Pressure(I2CDevice):
    ...
    def _read(self, command):
        ...
        try:
            ...
            raise OpenDCREException('No sensor reading returned from I2C.')

        except Exception:
            raise OpenDCREException('Error reading pressure sensor (device id: {})'.format(device_id)), None, sys.exc_info()[2]
                ...
                try:
                    ...
                    if vec.GetAck() == ACK:
                        ...
                        if not self._crc8_check(sense_data):
                            raise OpenDCREException('Sensor CRC check failed.')

                        return {const.UOM_PRESSURE: self._convert_reading(sense_data[0] + sense_data[1], self.altitude)}
                    else:
                        vec.Stop()
                        raise OpenDCREException('I2C Bus Error: No ACK received on device initialization.')
                finally:
                    vec.Close()
                    gpio.Close()
