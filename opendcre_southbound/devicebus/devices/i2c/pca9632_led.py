#!/usr/bin/env python
""" OpenDCRE Southbound I2C LED control via PCA9632
...
class PCA9632Led(I2CDevice):
    ...
    def _read(self, command):
        ...
        if led_state is not None and (color is None or blink is None):
            raise OpenDCREException('Color and blink must be specified on Vapor LED {} commands.'.format(led_state))
        ...
        try:
            ...
            raise OpenDCREException('No LED status returned from I2C.')

        except Exception:
            raise OpenDCREException('Error reading LED status (device id: {})'.format(device_id)), None, sys.exc_info()[2]
    ...
    def _chamber_led(self, command):
        ...
        try:
            ...
            raise OpenDCREException('No LED control response from I2C.')

        except Exception:
            raise OpenDCREException('Error setting LED status (device id: {})'.format(device_id)), None, sys.exc_info()[2]
        ...
        with self._lock:
            if self.hardware_type == 'emulator':
                ...
                if led_state == PCA9632_LEDOUT_OFF:
                    ...
                    raise OpenDCREException('Invalid LED State returned from LED controller: {}'.format(ord(led_state)))
