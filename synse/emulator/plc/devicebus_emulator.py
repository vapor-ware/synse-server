                                    ...
                                    if responses_length > 0 and _retry_count < responses_length:
                                        if isinstance(device["read"]["retry_responses"][_retry_count], list):
                                            ...
                                            logger.debug(">>Read: " + str(
                                                    [hex(x) for x in
                                                     transform_raw(raw_bytes=device
                                                                   ['read']['retry_responses'][_retry_count],
                                                     seq_no=sequence)]))
                                            emulator.write(
                                                    transform_raw(raw_bytes=device
                                                                  ["read"]["retry_responses"][_retry_count],
                                ...
                                if responses_length > 0 and _retry_count < responses_length:
                                    if isinstance(device["power"]["retry_responses"][_retry_count], list):
                                        ...
                                        logger.debug(">>Power: " + str(
                                                [hex(x) for x in
                                                 transform_raw(raw_bytes=device
                                                               ['power']['retry_responses'][_retry_count],
                                                 seq_no=sequence)]))
                                        emulator.write(
                                                transform_raw(raw_bytes=device
                                                              ["power"]["retry_responses"][_retry_count],
                            ...
                            if is_retry and ("retry_responses" in device["asset_info"]):
                                ...
                                if responses_length > 0 and _retry_count < responses_length:
                                    if isinstance(device["asset_info"]["retry_responses"][_retry_count], list):
                                        # we are sending raw bytes
                                        logger.debug(">>Asset Info: " + str(
                                                [hex(x) for x in
                                                 transform_raw(raw_bytes=device['asset_info']['retry_responses']
                                                               [_retry_count], seq_no=sequence)]))
                                        emulator.write(
                                                transform_raw(raw_bytes=device["asset_info"]["retry_responses"]
                                                              [_retry_count], seq_no=sequence))
                                        emulator.flush()
                                ...
                                if responses_length > 0 and _count < responses_length:
                                    if isinstance(device["asset_info"]["responses"][_count], list):
                                        ...
                                        logger.debug(
                                                ">>Asset Info: " + str(
                                                        [hex(x) for x in
                                                         transform_raw(raw_bytes=device['asset_info']['responses']
                                                                       [_count], seq_no=ac.sequence)]))
                                        emulator.write(
                                                transform_raw(raw_bytes=device["asset_info"]["responses"][_count],
                                                              seq_no=ac.sequence))
                                        emulator.flush()
                            ...
                            if is_retry and ("retry_responses" in device["boot_target"]):
                                ...
                                if responses_length > 0 and _retry_count < responses_length:
                                    if isinstance(device["boot_target"]["retry_responses"][_retry_count], list):
                                        # we are sending raw bytes
                                        logger.debug(">>Boot Target: " + str(
                                                [hex(x) for x in
                                                 transform_raw(raw_bytes=device['boot_target']['retry_responses']
                                                               [_retry_count], seq_no=sequence)]))
                                        emulator.write(
                                                transform_raw(raw_bytes=device["boot_target"]["retry_responses"]
                                                              [_retry_count], seq_no=sequence))
                                    ...
                                    if responses_length > 0 and _count < responses_length:
                                        if isinstance(device["boot_target"]["responses"][_count], list):
                                            ...
                                            logger.debug(
                                                    ">>Boot Target: " + str(
                                                            [hex(x) for x in
                                                             transform_raw(raw_bytes=device['boot_target']['responses']
                                                                           [_count], seq_no=pc.sequence)]))
                                            emulator.write(
                                                    transform_raw(raw_bytes=device["boot_target"]["responses"][_count],
                                                                  seq_no=pc.sequence))
                                            emulator.flush()
                                ...
                                if responses_length > 0 and _retry_count < responses_length:
                                    if isinstance(device["write"]["retry_responses"][_retry_count], list):
                                        ...
                                        logger.debug(">>Write: " + str(
                                                [hex(x) for x in
                                                 transform_raw(raw_bytes=device
                                                               ['write']['retry_responses'][_retry_count],
                                                 seq_no=sequence)]))
                                        emulator.write(
                                                transform_raw(raw_bytes=device
                                                              ["write"]["retry_responses"][_retry_count],
                            ...
                            if is_retry and ("retry_responses" in device["host_info"]):
                                ...
                                if responses_length > 0 and _retry_count < responses_length:
                                    if isinstance(device["host_info"]["retry_responses"][_retry_count], list):
                                        # we are sending raw bytes
                                        logger.debug(">>Host_Info: " + str(
                                                [hex(x) for x in
                                                 transform_raw(
                                                         raw_bytes=device['host_info']['retry_responses'][_retry_count],
                                                         seq_no=sequence)]))
                                        emulator.write(
                                                transform_raw(raw_bytes=device["host_info"]["retry_responses"]
                                                              [_retry_count], seq_no=sequence))
                                        emulator.flush()
                                    else:
                                        # otherwise, we do not send a response
                                ...
                                if responses_length > 0 and _count < responses_length:
                                    if isinstance(device["host_info"]["responses"][_count], list):
                                        ...
                                        logger.debug(
                                                ">>Host_Info: " + str(
                                                        [hex(x) for x in
                                                         transform_raw(raw_bytes=device['host_info']['responses']
                                                                       [_count], seq_no=lc.sequence)]))
                                        emulator.write(
                                                transform_raw(raw_bytes=device["host_info"]["responses"][_count],
                                                              seq_no=lc.sequence))
                                        emulator.flush()
