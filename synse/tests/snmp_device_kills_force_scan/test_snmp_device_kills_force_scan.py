...
EXPECTED_NO_BOARD = 'Board {} should not be there after scan all force since the emulator is dead.'
...
class SnmpDeviceKillsForceScanTestCase(SynseHttpTest):
    """ This test brings up two SNMP emulators for TestDevice1. The goal here
    is to allow the Synse container to initialize successfully with emulators
    ...
    over after Synse initialization and a forced scan is done to account for
    missing boards.
    """
    ...
    def test_scan_all_force(self):
        ...
        """
        # This may take a little longer for the SNMP calls to the dead emulators to time out,
        # so upped the timeout.
        r = http.get(Uri.create(_S.URI_SCAN_FORCE), timeout=15)
        self.assertTrue(http.request_ok(r.status_code))

        response = r.json()
        logger.debug(json.dumps(response, sort_keys=True, indent=4, separators=(',', ': ')))

        self._verify_valid_scan_all(response, 1)
    ...
    def test_scan_by_rack_and_board(self):
        """ Test scan rack_1, board 60000000 expecting sad results. Emulator is
        down and we have done a scan all force, so the board no longer exists
        to Synse."""
        try:
            http.get(Uri.create(_S.URI_SCAN, RACK_1, BOARD_60000000))
            self.fail(EXPECTED_VAPOR_HTTP_ERROR)
        except VaporHTTPError as e:
            self._verify_vapor_http_error(e, 500, _S.ERROR_NO_RACK_WITH_ID.format(RACK_1))
