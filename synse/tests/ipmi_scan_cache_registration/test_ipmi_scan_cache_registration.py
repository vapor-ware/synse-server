#!/usr/bin/env python
""" Synse IPMI Scan Cache Registration Tests
...
class IPMIScanCacheRegistrationTestCase(unittest.TestCase):
    ...
    For reference, the Synse config used for these tests (see data/synse_config.json):
      {
    ...
    @classmethod
    def setUpClass(cls):
        if not os.path.exists('/tmp/synse'):
            os.makedirs('/tmp/synse')

        cls.scan_cache = '/tmp/synse/cache.json'

        with open('/synse/synse/tests/data/scan_cache.json', 'r') as f:
