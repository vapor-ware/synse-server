...
class FanController(RS485Device):
    ...
    def _max_route(self, command):
        """Handle the max route that finds the maximum and minimum non-zero fan
        ...
        """
        # synse-server-internal only (non-doc) route to get the max fan speed in rpm.
        # supported by the motor. The max fan speed varies by motor and controller.
        ...
        # We could doc this route, for now it's not documented to limit support requirements.
        if not RS485Device.is_vec_leader():
            ...
            # We are vec_leader. We have the data already.
            reading = {
                const.UOM_VAPOR_FAN_MIN: self.min_nonzero_rpm,
                const.UOM_VAPOR_FAN_MAX: self.max_rpm,
            }
